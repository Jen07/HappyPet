...
const getImages = () => {
   ...
   content.innerHTML = '<td colspan="3" class="ta-center"  style="cursor: progress;">Cargando...</td>';
    ...
    xhr.open("GET", `/imagesAliment/get_product_images?id=${id}`, true);
    xhr.send();
    ...
    xht.open("GET", `/imagesAliment/get_id?id=${id}`, true);
    xht.send();
}
...
function prepareDeleate(id) {
    ...
    fetch(`/imagesAliment/deleteI?id=${idIma}`).then((response) => {
        resetTable();
    })
}
